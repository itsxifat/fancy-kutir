...
const ProductCard = ({ product, highlightName, highlightCategory }) => {
  ...
  return (
    ...
    >
      ...
      <p
        ...
        dangerouslySetInnerHTML={{ __html: highlightName || product.name }}
      />

      {/* Highlighted category if provided */}
      {highlightCategory && (
        <p
          className="w-full text-xs text-gray-500/70 max-sm:hidden truncate"
          dangerouslySetInnerHTML={{ __html: highlightCategory }}
        />
      )}

      {/* Fallback to description if no category highlight */}
      {!highlightCategory && (
